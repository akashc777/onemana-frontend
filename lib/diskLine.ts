 ...
 * workspace second. It never says "full" or "limit": the workspace keeps
 * working, and the sentence must not read as a block.
 */
...
export function diskLine(pct: number | undefined): string {
  ...
  if (p >= DISK_ATTENTION_PCT) {
    return `Disk ${p}% used. Remove files and messages you no longer need, or get in touch about a larger workspace.`;
  }
  return `Disk ${p}% used`;
}
