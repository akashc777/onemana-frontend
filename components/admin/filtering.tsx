...
export function FilterBar({
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2">
      ...
      {active && (
        <button onClick={() => onChange(emptyFilter)} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-slate-300 hover:bg-white/10">
          Clear
        </button>
      )}
      {typeof count === "number" && <span className="ml-auto text-xs text-slate-500">{count} result{count === 1 ? "" : "s"}</span>}
    </div>
  );
}
